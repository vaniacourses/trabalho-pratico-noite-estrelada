...
export class EmprestimoService {
  ...
  private limiteMáximoEmprestimos = 5; // Máximo de empréstimos simultâneos
  ...
  async realizarEmprestimo(
    ...
    const leitorVálido = await this.repository.verificarLeitorVálido(idLeitor);
    ...
    if (emprestimoAtivos >= this.limiteMáximoEmprestimos) {
      throw this.criarErro(
        "LIMITE_EMPRESTIMOS_ATINGIDO",
        `Limite de ${this.limiteMáximoEmprestimos} empréstimos simultâneos atingido`,
        400
      );
    }
